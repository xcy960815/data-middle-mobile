...
function getErrorMessage(error: unknown): string {
  return error instanceof Error ? error.message : '加载数据集失败，请稍后重试。';
}

function appendUniqueDatasets(
  currentItems: readonly DatasetListItem[],
  nextItems: readonly DatasetListItem[],
): DatasetListItem[] {
...
export function useDatasetList({ onUnauthorized }: UseDatasetListOptions = {}) {
  ...
  const [sort, setSort] = useState<DatasetSort>({ field: 'updateTime', order: 'desc' });
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (controller.signal.aborted || currentRequest !== requestVersion.current) return;
      setItems((currentItems) => appendUniqueDatasets(currentItems, response.list));
      setTotal(response.total);
